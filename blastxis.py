...
def get_args():
    ...
    parser.add_argument('-cpu' , '--num_threads_dfast', type=int, default=48,
                       help='num threads in dfast.(default:48)') 
...
class MyGetIS(object):
    ...
    def get_1st_interval(cls, cds_i, 
        ...
        id_out += [f'interval_edge_region{counter}_LEN{len(tmp)}_{genome_i.id}_{start}_{end}']
        return counter
    ...
    def get_last_interval(cls, cds_h, 
        ...
        seq_out += [tmp] ;counter+=1
        id_out += [f'interval_edge_region{counter}_LEN{len(tmp)}_{genome_h.id}_{start}_{end}']
        return counter
